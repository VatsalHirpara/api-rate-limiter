...
import io.vatsal.ratelimiter.dto.Counter;
...
public class RateLimitingServiceImpl {
    ...
    /**
     * Map <userId, Map< minute in epoch, count>>
     * uuid-123 : {
     * 1688232: 1,
     * 1688260: 3,
     * }
    ...
    public boolean isLimitExceeded(String userId) throws InterruptedException {
        if (rateLimiter.containsKey(userId)) {
            ...
            final long currentTime = getStartOfCurrentMinute();
            rateLimiter.put(userId, new HashMap<Long, Long>() {{
                put(currentTime, 1L);
            }});
        }
    ...
    private long getStartOfCurrentMinute() {
        ...
        final LocalDateTime startOfMinute = currentTime.minusSeconds(currentTime.getSecond());
